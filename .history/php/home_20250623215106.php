...
mysqli_select_db($conn, $base) or die(header("Location: install.php#t1"));
...
$user = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM users WHERE id_users = '$id_p'"));
$profile_image = !empty($user['pp']) ? '../Images/pp_users/' . $user['pp'] : '../Images/default_profile.png';

$stats = [
    'users' => getCount($conn, "SELECT COUNT(*) FROM users"),
    'reservations' => getCount($conn, "SELECT COUNT(*) FROM reservation"),
    'salles' => getCount($conn, "SELECT COUNT(*) FROM salle"),
    'salles_dispo' => getCount($conn, "SELECT COUNT(*) FROM salle WHERE disponibilite = 'disponible'"),
    'salles_indispo' => getCount($conn, "SELECT COUNT(*) FROM salle WHERE disponibilite != 'disponible'"),
    'notif' => getCount($conn, "SELECT COUNT(*) FROM reservation WHERE status = 'on'"),
    'signalements' => getCount($conn, "SELECT COUNT(*) FROM service"),
];
$numnot = getCount($conn, "SELECT COUNT(*) FROM reservation WHERE status = 'on'")
    + getCount($conn, "SELECT COUNT(*) FROM service WHERE status = 'nouveau'");

$res_week_reservations = mysqli_query($conn, "
    SELECT * FROM reservation 
    WHERE YEARWEEK(date_res, 1) = YEARWEEK(CURDATE(), 1)
    ORDER BY date_res ASC, debh ASC
");
$res_latest_notif = mysqli_query($conn, "SELECT * FROM reservation ORDER BY date_res DESC LIMIT 5");
$res_latest_signal = mysqli_query($conn, "SELECT * FROM service WHERE action = '' ORDER BY id_service DESC LIMIT 5");
?>
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative tooltip" onclick="toggleUserMenu()" onmouseover="toggleTooltip('user-tooltip')"
                    ...
                    <div id="userMenu" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg hidden z-50">
                        <div class="py-1">
                            <a href="usere.php" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                ...
                <div
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-800"><?php echo $stats['salles']; ?></p>
                            <p class="text-sm text-gray-500">Disponibles: <?php echo $stats['salles_dispo']; ?></p>
                            <p class="text-sm text-gray-500">Non disponibles: <?php echo $stats['salles_indispo']; ?>
                            </p>
                        </div>
                    ...
                    <div class="mt-4">
                        <a href="maintenance.php" class="text-primary text-sm hover:underline">
                            Voir détails <i class="fas fa-arrow-right ml-1"></i>
                        </a>
                    </div>
                </div>
                <div
                    ...
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-bold text-gray-800"><?php echo $stats['notif']; ?></p>
                        </div>
